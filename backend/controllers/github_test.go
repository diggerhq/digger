...
func TestJobsTreeWithOneJobsAndTwoProjects(t *testing.T) {
	...
	_, result, err := utils.ConvertJobsToDiggerJobs(jobs, projectMap, graph, 41584295, "", 2, "diggerhq", "parallel_jobs_demo", "diggerhq/parallel_jobs_demo", "test")
	assert.NoError(t, err)
...
func TestJobsTreeWithTwoDependantJobs(t *testing.T) {
	...
	_, result, err := utils.ConvertJobsToDiggerJobs(jobs, projectMap, graph, 123, "", 2, "", "", "test", "test")
	assert.NoError(t, err)
