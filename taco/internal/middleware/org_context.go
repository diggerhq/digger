...
func ResolveOrgContextMiddleware(resolver domain.IdentifierResolver) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			orgUUID, err := resolver.ResolveOrganization(c.Request().Context(), orgName)
			if err != nil {
				log.Printf("[Middleware] Failed to resolve organization '%s': %v", orgName, err)
			...
			if err != nil {
				log.Printf("[Middleware] Failed to get org details for %s: %v - falling back to basic context", orgUUID, err)
