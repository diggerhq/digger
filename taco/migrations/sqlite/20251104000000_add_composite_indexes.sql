...
CREATE INDEX IF NOT EXISTS "idx_units_org_id_name" ON "units" ("org_id", "name");

-- Add composite index for role lookups by org and name
CREATE INDEX IF NOT EXISTS "idx_roles_org_id_name" ON "roles" ("org_id", "name");

-- Add composite index for permission lookups by org and name  
CREATE INDEX IF NOT EXISTS "idx_permissions_org_id_name" ON "permissions" ("org_id", "name");

-- Add composite index for tag lookups by org and name
CREATE INDEX IF NOT EXISTS "idx_tags_org_id_name" ON "tags" ("org_id", "name");
...
-- SQLite can use these indexes for both exact matches and prefix searches.
