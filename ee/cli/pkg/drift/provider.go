...
func (d DriftNotificationProviderAdvanced) Get(prService ci.PullRequestService) (core_drift.Notification, error) {
	...
	msTeamsNotificationUrl := os.Getenv("INPUT_DRIFT_DETECTION_MS_TEAMS_NOTIFICATION_URL")
	DriftAsGithubIssues := os.Getenv("INPUT_DRIFT_GITHUB_ISSUES")
	var notification core_drift.Notification
	if msTeamsNotificationUrl != "" {
		notification = &ce_drift.MSTeamsNotification{Url: msTeamsNotificationUrl}
	} else if slackNotificationUrl != "" {
		...
		return nil, fmt.Errorf("could not identify drift mode, please specify using env variable INPUT_DRIFT_DETECTION_MS_TEAMS_NOTIFICATION_URL, INPUT_DRIFT_DETECTION_SLACK_NOTIFICATION_URL, INPUT_DRIFT_DETECTION_ADVANCED_SLACK_NOTIFICATION_URL or INPUT_DRIFT_GITHUB_ISSUES")
	}
