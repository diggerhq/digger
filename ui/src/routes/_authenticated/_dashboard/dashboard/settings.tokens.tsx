...
function RouteComponent() {
  ...
  const [pageSize] = useState<number>(tokens?.page_size ?? 20)
  ...
  const tokenList = tokenPage?.tokens ?? []
  const totalTokens = tokenPage?.total ?? tokenList.length
  const effectivePage = tokenPage?.page ?? currentPage
  const effectivePageSize = tokenPage?.page_size ?? pageSize
  const canGoNext = effectivePage * effectivePageSize < totalTokens
  const canGoPrev = effectivePage > 1

  const refreshTokens = async (page: number = currentPage) => {
    const next = await getTokensFn({
      data: { organizationId: organisationId, userId: user?.id || '', page, pageSize },
    })
    setTokenPage(next)
  ...
  const handleRevokeToken = async (tokenId: string) => {
    deleteTokenFn({data: {organizationId: organisationId, userId: user?.id || '', tokenId: tokenId}}).then(() => {
      ...
      setSubmitting(false)
      await refreshTokens(currentPage)
    })
  }
  return (
    <Card>
            ...
            <>
              ...
              <div className="flex items-center justify-between pt-3">
                ...
                <div className="space-x-2">
                  <Button variant="outline" size="sm" disabled={!canGoPrev} onClick={() => refreshTokens(effectivePage - 1)}>
                    Previous
                  </Button>
                  <Button variant="outline" size="sm" disabled={!canGoNext} onClick={() => refreshTokens(effectivePage + 1)}>
                    Next
                  </Button>
                </div>
              </div>
