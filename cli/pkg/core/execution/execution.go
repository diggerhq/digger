...
func reportError(r reporting.Reporter, stderr string) {
	if r.SupportsMarkdown() {
		_, commentErr := r.Report(stderr, utils.AsCollapsibleComment("Error during init.", false))
		if commentErr != nil {
			log.Printf("error publishing comment: %v", commentErr)
		}
	} else {
		_, commentErr := r.Report(stderr, utils.AsComment("Error during init."))
		if commentErr != nil {
			log.Printf("error publishing comment: %v", commentErr)
		}
	}
...
func reportApplyError(r reporting.Reporter, err error) {
	if r.SupportsMarkdown() {
		_, commentErr := r.Report(err.Error(), utils.AsCollapsibleComment("Error during applying.", false))
		if commentErr != nil {
			log.Printf("error publishing comment: %v", err)
		}
	} else {
		_, commentErr := r.Report(err.Error(), utils.AsComment("Error during applying."))
		if commentErr != nil {
			log.Printf("error publishing comment: %v", err)
		}
	}
...
func reportTerraformApplyOutput(r reporting.Reporter, projectId string, applyOutput string) {
	...
	_, commentErr := r.Report(applyOutput, formatter)
	if commentErr != nil {
		log.Printf("error publishing comment: %v", commentErr)
	}
}
...
func reportTerraformError(r reporting.Reporter, stderr string) {
	if r.SupportsMarkdown() {
		_, commentErr := r.Report(stderr, utils.GetTerraformOutputAsCollapsibleComment("Error during init.", false))
		if commentErr != nil {
			log.Printf("error publishing comment: %v", commentErr)
		}
	} else {
		_, commentErr := r.Report(stderr, utils.GetTerraformOutputAsComment("Error during init."))
		if commentErr != nil {
			log.Printf("error publishing comment: %v", commentErr)
		}
	}
...
func reportAdditionalOutput(r reporting.Reporter, projectId string) {
	...
	if _, err := os.Stat(diggerOutPath); err == nil {
		...
		if len(outputStr) > 0 {
			_, commentErr := r.Report(outputStr, formatter)
			if commentErr != nil {
				log.Printf("error publishing comment: %v", commentErr)
			}
		} else {
