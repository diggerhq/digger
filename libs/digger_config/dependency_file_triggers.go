...
func getDependencyFileTriggerPatterns(repoRoot string, project Project) ([]string, error) {
	...
	case project.Pulumi:
		return nil, nil
	case project.Terragrunt:
		return getTerragruntDependencyFileTriggerPatterns(repoRoot, project.Dir)
	default:
...
func getTerragruntDependencyFileTriggerPatterns(repoRoot string, projectDir string) ([]string, error) {
	whenModified, err := tac.InferProjectWhenModifiedPatterns(repoRoot, projectDir, false, false, true, false)
	if err != nil {
		return nil, err
	}
	...
	patterns, err := GetPatternsRelativeToRepo(projectDir, whenModified)
	if err != nil {
		return nil, err
	}
