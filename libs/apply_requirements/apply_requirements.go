	...
	for _, proj := range impactedProjects {
		for _, req := range proj.ApplyRequirements {
			if req == digger_config.ApplyRequirementsApproved && isApproved == false {
				return fmt.Errorf("PR fails apply requirements for project %v, Expected PR to be approved", proj.Name)
			} else if req == digger_config.ApplyRequirementsUndiverged && isDiverged == true {
				return fmt.Errorf("PR fails apply requirements for project %v, Expected PR to be undiverged from target branch", proj.Name)
			} else if req == digger_config.ApplyRequirementsMergeable && isMergeable == false {
				return fmt.Errorf("PR fails apply requirements for project %v, Expected PR to be mergable", proj.Name)
			} else {
