...
var unlockCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if len(args) > 1 {
            ...
            lockID = getLockID(stateID)
            if lockID == "" {
                return fmt.Errorf("no lock ID provided and none found for %s", stateID)
            }
        }
...
func getLockDir() string { return ".taco" }
...
func saveLockID(stateID, lockID string) {
    os.MkdirAll(getLockDir(), 0o755)
    lockFile := getLockFile(stateID)
    os.WriteFile(lockFile, []byte(lockID), 0o644)
}
