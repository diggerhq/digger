...
OpenTaco supports using a query backend to speed up the retrieval of objects from S3. By default SQLite will initialize, but other SQL databases can be configured if desired.
...
SQLite is the default query backend and requires no external database server.
...
- All passwords should be set via environment variables, never hardcoded
