...
Here's how to do a quickstart for OpenTaco with Terraforn Cloud Block, Auth0, and S3.
...
OpenTaco uses two separate authentication flows that serve different purposes and must be completed in sequence:

#### Step 1: Taco CLI Authentication

First, authenticate the taco CLI for management operations:

```bash
taco login --server https://your-opentaco-server.com
```
...
4. Store access/refresh tokens in `~/.config/opentaco/credentials.json`

**What this enables:** Unit management commands (`taco unit ls`, `taco unit status`, etc.)

#### Step 2: Terraform Cloud Block Authentication

Next, authenticate Terraform for cloud block operations:

```bash
terraform login <your-opentaco-hostname>
```

This command will:
1. Use the same OAuth flow as taco login
2. Store Terraform-specific credentials in `~/.terraform.d/credentials.tfrc.json` 
3. Enable cloud block functionality in your Terraform configurations

**What this enables:** Terraform operations using the `cloud` block (`terraform init`, `terraform plan`, `terraform apply`)

#### Why Two Separate Authentication Steps?

- **Different credential stores**: Taco CLI and Terraform store credentials in separate files
- **Different scopes**: Each authentication flow is optimized for its specific use case
- **Independent operation**: You can use Terraform cloud block operations even if taco CLI authentication expires, and vice versa

Both authentications use the same OIDC identity provider and will result in the same user context and permissions.
...
After completing both authentication steps, you can verify each authentication separately:

#### Verify Taco CLI Authentication
```bash
# Check taco CLI authentication status
taco whoami

# List available workspaces (requires taco login)
taco unit ls

# Check workspace status (requires taco login)
taco unit status my-workspace
```

#### Verify Terraform Authentication
```bash
# Test Terraform cloud block functionality (requires terraform login)
terraform init
terraform plan
```

**Note:** The `taco unit` commands require `taco login` credentials, while `terraform` commands with cloud block require `terraform login` credentials.
...
### Step 1: Authenticate Both Systems
...
### Step 8: Make Changes and Update
...
### Step 9: Explore Unit Management
