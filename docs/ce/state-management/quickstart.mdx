    ...
    <Step title="Setup Auth provider">
        <Tabs>
            <Tab title="Auth0">
                ...
                ```bash
                OPENTACO_AUTH_ISSUER=[[auth0 domain]]
                OPENTACO_AUTH_CLIENT_ID=[[auth0 client id]]
                OPENTACO_AUTH_CLIENT_SECRET=[[auth0 client secret]]
                ```

                In the settings tab scroll down to callback urls and add the following callback url: `http://127.0.0.1:8585/callback` and hit save changes
...
<Steps >
    ...
    <Step title="Configure CLI Environment Variables">
        ...
        <Tabs>
            ...
            export OPENTACO_AUTH_ISSUER=[[auth0 domain]]
