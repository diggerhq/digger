...
<AccordionGroup>
  <Accordion title="Block Configuration">
    ...
    <ParamField path="terragrunt_parallelism" type="integer">
      Caps the worker pool Terragrunt uses for dependency loading on each unit plan/apply — equivalent to passing `-parallelism=N` to Terragrunt. Useful for large estates where unbounded concurrency overwhelms the runner. Must be between `1` and `20`. Only applies when the generated projects are Terragrunt.
    </ParamField>
