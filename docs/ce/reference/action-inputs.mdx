...
inputs:
  ...
  google-lock-bucket:
    ...
    required: false
  alicloud-lock-bucket:
    description: The Alibaba Cloud OSS bucket to use for PR locks in backendless mode (`no-backend: true`). Requires `LOCK_PROVIDER: alicloud`
    required: false
  alicloud-region:
    description: Alibaba Cloud region of the OSS buckets used for locks and plan artifacts, e.g. cn-hangzhou
    required: false
  setup-azure:
  ...
  upload-plan-destination:
    description: Destination to upload the plan to. azure, gcp, github, aws and alicloud are currently supported.
    required: false
  ...
  upload-plan-destination-gcp-bucket:
    ...
    required: false
  upload-plan-destination-alicloud-bucket:
    description: Name of the destination Alibaba Cloud OSS bucket. Should be provided if destination == alicloud
    required: false
  setup-checkov:
