...
Credentials are read from the environment: either `OSS_ACCESS_KEY_ID` / `OSS_ACCESS_KEY_SECRET`, or the standard `ALIBABA_CLOUD_*` variables (access keys, OIDC role or ECS RAM role) that the Terraform alicloud provider already uses. See [Alibaba Cloud: Backendless OSS lock storage](/ce/alicloud/using-oss-bucket-for-locks) for details.
