...
def lock_project(dynamodb, repo_name, pr_number, token, project_name=""):
    ...
    print(f"lock_project {project_name}, lock:{lock}")
    if lock:
    ...
    if lock_acquired:
        comment = f"Project {project_name} has been locked by PR #{pr_number}"
        pull_request.publish_comment(comment)
        print(f"project {project_name} locked successfully. PR #{pr_number}")
